...
class Scheduler:
    def __init__(self, problem_ids: list[str], state_manager: StateManager):
        self.active_problem_ids = list(problem_ids)
    ...
    def update_problem(self, problem_id: str, is_ac: bool, time_spent: float) -> None:
        """Updates the state and pushes the problem back into the queue."""
        if problem_id not in self.active_problem_ids:
            return
